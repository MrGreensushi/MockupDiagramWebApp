...
type ActivityNodeProps = {
  label: string;
  activity: Activity;
  onClickEdit: (activity: Activity | undefined, setActivity: React.Dispatch<React.SetStateAction<Activity| undefined>>) => void;
...
function ActivityNode(props: NodeProps<ActivityNodeType>) {
  ...
  return (
    <div className={`scene-node ${props.selected ? "selected" : ""}`}>
      ...
      <NodeToolbar>
        <ButtonGroup>
          <Button variant="secondary" onClick={() => props.data.onClickEdit(activity, setActivity)}>
